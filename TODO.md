...
- [ ] **Generated narrative `<h1>` reads "Five Mornings in " — the county name is missing.** The heading is produced by the template engine in `pkg/narrative/`, so the placeholder is not being substituted. User-visible on every generated narrative; the frontend renders exactly what the API returns [source: narrative track, observed in-browser]
